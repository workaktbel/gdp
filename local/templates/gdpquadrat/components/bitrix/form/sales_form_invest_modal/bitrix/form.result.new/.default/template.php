					...
					<?foreach($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion):?>
						...
						<?elseif($class != 'type_checkbox'):?>
							...
							<div class="tpl-field <?=$type;?> popup-form__form-field required">
								<input type="<?=$arQuestion['STRUCTURE'][0]['FIELD_TYPE']?>" placeholder="<?=$arQuestion["CAPTION"]?>" type="<?=$arQuestion['STRUCTURE'][0]['FIELD_TYPE']?>" <?=$arQuestion['STRUCTURE'][0]['FIELD_PARAM']?> <?= $req ?> id="form_<?=$arQuestion['STRUCTURE'][0]['FIELD_TYPE']?>_<?=$arQuestion['STRUCTURE'][0]['ID']?>" name="form_<?=$arQuestion['STRUCTURE'][0]['FIELD_TYPE']?>_<?=$arQuestion['STRUCTURE'][0]['ID']?>" value="">
								<?if (is_array($arResult["FORM_ERRORS"]) && array_key_exists($FIELD_SID, $arResult['FORM_ERRORS'])):?>
